...
static const int codec_ids[4] = { 0, CODEC_ID_VP8, CODEC_ID_RV40, CODEC_ID_SVQ3 };
...
static void check_pred8x8l(H264PredContext *h, uint8_t *buf0, uint8_t *buf1,
                           int codec, int chroma_format, int bit_depth)
{
    if (chroma_format == 1 && codec_ids[codec] == 0) {
        int pred_mode;
...
void checkasm_check_h264pred(void)
{
    ...
    H264PredContext h;
    size_t test;
    int codec, chroma_format, bit_depth;

    for (test = 0; test < FF_ARRAY_ELEMS(tests); test++) {
        for (codec = 1; codec < 2; codec++) {
            int codec_id = codec_ids[codec];
            for (bit_depth = 8; bit_depth <= 8; bit_depth++)
                for (chroma_format = 1; chroma_format <= 1; chroma_format++) {
                    ff_h264_pred_init(&h, codec_id, bit_depth);
