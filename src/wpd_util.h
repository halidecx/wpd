...
#define WPD_ARRAY_ELEMS(a) (sizeof(a) / sizeof((a)[0]))
#define WPD_INT_BITS (sizeof(int) * CHAR_BIT)
#define WPD_SWAP(type, a, b) do { type wpd_swap = (a); (a) = (b); (b) = wpd_swap; } while (0)
#define wpd_uninit(x) x

#define WPD_ERROR(e) (-(e))
#define WPD_ERROR_INVALID_DATA (-1094995529)
#define WPD_LOG_ERROR 0
#define WPD_LOG_WARNING 1
#define WPD_LOG_FATAL 2

static wpd_always_inline int wpd_clip(int value, int low, int high)
...
static wpd_always_inline uint16_t wpd_r16(const void *p)
...
static wpd_always_inline void wpd_w16(void *p, uint16_t v) { memcpy(p, &v, 2); }
...
#define WPD_RN16(p) wpd_r16(p)
#define WPD_RN32(p) wpd_r32(p)
#define WPD_RN32A(p) wpd_r32(p)
#define WPD_WN16(p, v) wpd_w16(p, v)
#define WPD_WN32(p, v) wpd_w32(p, v)
...
#define WPD_RL32(p) ((uint32_t)((p)[0] | (p)[1] << 8 | (p)[2] << 16 | (uint32_t)(p)[3] << 24))
#define WPD_COPY32(d, s) wpd_w32(d, wpd_r32(s))
#define WPD_COPY64(d, s) wpd_w64(d, wpd_r64(s))
#define WPD_COPY128(d, s) memcpy(d, s, 16)
#define WPD_ZERO32(d) memset(d, 0, 4)
#define WPD_ZERO64(d) memset(d, 0, 8)
#define WPD_ZERO128(d) memset(d, 0, 16)
#define WPD_SWAP64(a, b) do { uint64_t wpd_v = wpd_r64(a); WPD_COPY64(a, b); wpd_w64(b, wpd_v); } while (0)
