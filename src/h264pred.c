...
static void pred4x4_horizontal_vp8_c(uint8_t *src, const uint8_t *topright, int stride){
    ...
    AV_WN32A(src+0*stride, ((lt + 2*l0 + l1 + 2) >> 2)*0x01010101u);
    AV_WN32A(src+1*stride, ((l0 + 2*l1 + l2 + 2) >> 2)*0x01010101u);
    AV_WN32A(src+2*stride, ((l1 + 2*l2 + l3 + 2) >> 2)*0x01010101u);
    AV_WN32A(src+3*stride, ((l2 + 2*l3 + l3 + 2) >> 2)*0x01010101u);
}
...
static void pred8x8_left_dc_rv40_c(uint8_t *src, int stride){
    ...
    for(i=0;i<8; i++)
        dc0+= src[-1+i*stride];
    dc0= (int)(0x01010101u*((unsigned)(dc0 + 4)>>3));
...
static void pred8x8_dc_rv40_c(uint8_t *src, int stride){
    ...
    dc0= (int)(0x01010101u*((unsigned)(dc0 + 8)>>4));
...
#if defined(FFVP8_H264PRED_ARM)
    ff_h264_pred_init_arm(h, codec_id, bit_depth);
#endif
#if defined(FFVP8_H264PRED_AARCH64)
    ff_h264_pred_init_aarch64(h, codec_id, bit_depth);
#endif
#if defined(FFVP8_H264PRED_X86)
    ff_h264_pred_init_x86(h, codec_id, bit_depth);
#endif
}
