...
static void pack_bgr565_c(uint8_t *dst, const uint8_t *src, int num_pixels) {
    for (int i = 0; i < num_pixels; i++) {
        const int r = src[4 * i + 1];
        const int g = src[4 * i + 2];
        const int b = src[4 * i + 3];

        dst[2 * i]     = (uint8_t)((g << 3 & 0xe0) | b >> 3);
        dst[2 * i + 1] = (uint8_t)((r & 0xf8) | g >> 5);
    }
}
...
static void pack_bgra4444_c(uint8_t *dst, const uint8_t *src, int num_pixels) {
    for (int i = 0; i < num_pixels; i++) {
        dst[2 * i]     = (uint8_t)((src[4 * i + 3] & 0xf0) | src[4 * i] >> 4);
        dst[2 * i + 1] = (uint8_t)((src[4 * i + 1] & 0xf0) |
                                   src[4 * i + 2] >> 4);
    }
}
...
static void premultiply_row_4444_swap_c(uint8_t *bgra4444, int num_pixels) {
    for (int i = 0; i < num_pixels; i++) {
        const unsigned ba   = bgra4444[2 * i];
        const unsigned rg   = bgra4444[2 * i + 1];
        const unsigned a    = ba & 0x0f;
        const unsigned mult = a * 0x1111u;
        const unsigned r    = (((rg & 0xf0) | rg >> 4) * mult) >> 16;
        const unsigned g    = (((rg & 0x0f) | (rg << 4 & 0xf0)) * mult) >> 16;
        ...
        bgra4444[2 * i]     = (uint8_t)((b & 0xf0) | a);
        bgra4444[2 * i + 1] = (uint8_t)((r & 0xf0) | (g >> 4 & 0x0f));
    }
}
